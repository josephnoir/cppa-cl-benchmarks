...
def benchmark(executable, iterations, size, postfix, samples, test):
    print("Running 'time {0} -i {1} -s {2}' for {3} samples".format(executable, iterations, size, samples))
    file_name = result_path + executable + postfix + ".dat"
    if not test:
        fd = open(file_name, "w")
    print("Output to: {}".format(file_name))
    for n in range(0, samples):
        rc = subprocess.check_output(["time", bin_path + executable, "-i", str(iterations), "-s", str(size)], 
...
def main():
    ...
    parser.add_option("-i", "--iterations"     , action="store", type="int", dest="iterations",
    ...
    parser.add_option("-e", "--end"            , action="store", type="int", dest="end",
                      help="make benchmarks for different sizes [size end]"       , default=0)
    parser.add_option("-a", "--increment"      , action="store", type="int", dest="increment",
                      help="increment size after each sample until end is reached", default=0)
    ...
    if options.end <= options.size:
        postfix = ""
        for e in args:
            benchmark(e, options.iterations, options.size, postfix, options.samples, options.test)
    elif options.end > options.size and options.increment > 0:
        sample_sizes = range(options.size, options.end + 1, options.increment)
        for e in args:
            for s in sample_sizes:
                postfix = "_" + repr(s)
                benchmark(e, options.iterations, s, postfix, options.samples, options.test)
    else:
